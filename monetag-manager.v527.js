/**
 * GameZipper Tools — Monetag Ad Manager v5.27-tools-hubonly-11012002 (Poki-style)
 * ───────────────────────────────────────────────────────────────────────────────────────
 * Poki-model: Smart frequency control, glass overlay + progress bar
 *
 * v5.27 Changes (2026-07-15 — 11012002 hub-only fix, kanban t_ed9d6e6f):
 *   - 🪲 Fix: 11012002 (cross-deployed from gamezipper.com) only fills on hub pages (/, /zh/),
 *     not on tools subdomain. BI 7d 2026-07-08~2026-07-15: tools 11012002 fill = 0, no_fill = 27,
 *     backoff_skip = 0. gz.com 11012002 7d: fill = 64, no_fill = 88, fill_rate = 42.1%.
 *     Root cause: Monetag zone 11012002 is registered for gz.com domain; tools subdomain
 *     receives no fill responses (Monetag checks referer/domain at fill time).
 *   - 🔧 Add hubPageOnly guard to 11012002 in showHomepageBanner() + showInPagePush():
 *     both already guarded by state.isHubPage, but the waterfall path in showHomepageBanner()
 *     calls startMonetagWaterfall() directly after AdSense fails — no double-check. Add
 *     isHubPage guard inside startMonetagWaterfall() to short-circuit on sub-pages.
 *   - 🔧 Add trace event 'hub_page_only_skip' for BI visibility. sub-page now gets
 *     explicit event instead of silent 3s timeout + no_fill.
 *   - 🔧 Add 11012002 to CONFIG.deadZones? NO — deadZones would skip on hub pages too,
 *     breaking the working 42.1% fill rate. hubPageOnly is the right guard.
 *   - 📊 Expected impact (BI 7d post-deploy):
 *     - tools 11012002 no_fill: 27/7d → 0 (sub-pages no longer call it)
 *     - tools 11012002 hub page fill: unchanged (still 5-7/week — working path)
 *     - tools homepage_banner_fill (11012002): 0/7d → 5-7/7d (only hub pages)
 *     - tools total 11012002 calls: 27 → 5-7/7d (hub-only)
 *   - 🛡️ Safety: AdSense Tier 0 unchanged. Revert by removing isHubPage guard in
 *     startMonetagWaterfall(). Backwards compatible: gz.com 11012002 unaffected.
 *   - Version bumped 5.26-tools-backoff-fresh-reset → 5.27-tools-hubonly-11012002.
 *
 ...
 *     shared/common.js). 7d BI 2026-07-03~07-09: tools 11012002 fills came
 *     exclusively from hub pages (/, /zh/, /html-cheat-sheet.html) — 0 fills
 *     on any sub-page despite ~30+ no_fill/load_error events. Root cause: 11012002
 *     is registered for gz.com in Monetag's zone config; tools subdomain never
 *     receives fill responses (Monetag checks referer/domain at fill time).
 *     Post-v5.23 (2026-07-09 05:30~09:14, 3.7h): 1 load_error, 1 inpage_push_no_fill,
 *     0 fills — confirming sub-page 11012002 is a 100% dead end.
 *   - 🔧 Guard showInPagePush() with state.isHubPage (same pattern as
 *     gz.com displayAd() — explicit call site). Sub-pages skip the call
 *     entirely, saving the 3s timeout × every-page-load wasted CPU + 11012002
 *     fill-cycle wear.
 *   - 🔧 Add 'inpage_push_fill' trackAdEvent on success — BI previously only
 *     logged inpage_push_no_fill, so successful fills were invisible in BI
 *     (hidden behind generic 'fill' event). Now 1:1 event parity with
 *     gz.com 'commercial_break_fill' style events.
 *   - ⚙️ ZONE_BACKOFF.backoffs: [10min, 30min, 2h] → [10min, 30min, 60min].
 *     Align with gamezipper.com v5.4 curve. tools sub-pages were getting
 *     locked out 2h after 3 failed attempts — way too long for a 30min
 *     session-window frequency cap (CONFIG.FREQUENCY.sessionWindowMs). 60min
 *     streak-3 lets the zone retry in the next user session.
 *   - 📊 Expected impact (BI 7d post-deploy):
 *       - tools 11012002 sub-page load_error: ~0.7/d → 0 (-100%) [no more 3s timeout per sub-page]
 *       - tools 11012002 sub-page inpage_push_no_fill: ~0.3/d → 0 (-100%)
 *       - tools 11012002 hub page fill: unchanged (still 3-5/day) — AdSense Tier 0 unchanged
 *       - tools 11012002 zone_backoff: 11 events/5d → 3-4 events/5d (less aggressive lockout)
 *       - Page load: save 3s timeout × ~200 sub-page-loads/day wasted = ~10min/day CPU
 *   - 🛡️ Safety: AdSense Tier 0 + showHomepageBanner + showContainerAd all unchanged.
 *     No new ad calls. Frequency caps untouched. Re-enable sub-page showInPagePush
 *     by removing the isHubPage guard (1-line revert) if Monetag later authorizes
 *     11012002 for tools subdomain.
 *   - Version bumped 5.23-tools-showInPagePush-cull → 5.24-tools-inpage-hub-only.
 *
...
;(function() {
  ...
  var CONFIG = {
    ...
    VERSION: '5.27-tools-hubonly-11012002',  // 2026-07-15: v5.27 kanban t_ed9d6e6f — 11012002 hub-only fix. BI 7d 2026-07-08~07-15: tools 11012002 fill=0/no_fill=27 vs gz.com 11012002 fill=64/no_fill=88 (42.1%). Zone registered for gz.com domain only, so tools subdomain gets no fill. Add hubPageOnly guard inside showHomepageBanner()'s startMonetagWaterfall() to skip on sub-pages. Pairs with showInPagePush() same guard. Expected: tools 11012002 no_fill 27/7d → 0, hub page fill unchanged (5-7/week).
    ...
    ZONE_BACKOFF: {
      ...
      backoffs: [10 * 60 * 1000, 30 * 60 * 1000, 60 * 60 * 1000],  // v5.24: align to gz.com v5.4 [10min, 30min, 60min] (was 2h streak-3, too long for 30min session window)
    ...
    //   profitabledisplaynetwork.com/{zoneId}.js returns HTTP 301→google.com.
    //   6d BI: 0 fills / 11 attempts. Default OFF — opt-in via window.GZ_ADSTERRA_ENABLED=true.
    //   Indexed adsterra tier (8 loadAdsterraZone call sites) was adding +6s/network per page.
    ADSTERRA: {
      ...
      (function(containerId, slotId) {
        ...
        setTimeout(function() {
          ...
          function startMonetagWaterfall() {
            if (container.getAttribute('data-filled')) return;
            // v5.27: 11012002 hub-only guard. BI 7d 2026-07-08~07-15: tools 11012002 fill=0/no_fill=27
            //   vs gz.com 11012002 fill=64/no_fill=88 (42.1%). Zone registered for gz.com domain only,
            //   so tools subdomain gets no fill. showInPagePush() already has isHubPage guard, but this
            //   waterfall path runs after AdSense fails on sub-pages too (no double-check). Skip it.
            if (!state.isHubPage) {
              trackAdEvent('hub_page_only_skip', { containerId: containerId, zoneId: ZONES.inpagePushGz });
              return;
            }
  ...
  // ==================== In-Page Push (轻量级, hub page only — v5.24) ====================
  // v5.24: Hub-page-only. 7d BI 2026-07-03~07-09: tools 11012002 fills came
  //   exclusively from hub pages (/, /zh/, /html-cheat-sheet.html) — 0 fills
  //   on any sub-page despite ~30+ no_fill/load_error events. Root cause: 11012002
  //   is registered for gamezipper.com in Monetag; tools subdomain gets no fill
  //   responses. Sub-pages now skip this call entirely (3s timeout saved per
  //   sub-page load). Re-enable sub-page support by removing the isHubPage guard.
  function showInPagePush() {
    if (!canShowAd()) return;
    if (!state.isHubPage) return;  // v5.24: skip sub-pages (11012002 not authorized for tools subdomain)
    setTimeout(function() {
      if (!canShowAd()) return;
      if (!state.isHubPage) return;  // double-check (state could change in setTimeout)
      ...
      // = 10689345 disabled + adsterra CDN-dead) was 100% dead ends on tools.
      loadZone(ZONES.inpagePushGz).then(function() {
        markShown();
        trackAdEvent('inpage_push_fill', { network: 'monetag', zoneId: ZONES.inpagePushGz });
      }).catch(function() {
