/**
 * GameZipper Tools — Monetag Ad Manager v5.27-tools-v526-bump (Poki-style)
 * ───────────────────────────────────────────────────────────────────────────────────────
 * Poki-model: Smart frequency control, glass overlay + progress bar
 *
 * v5.27 Changes (2026-07-14 — v526 file bump + cache buster hotfix, kanban t_40a17bfc):
 *   - 🪲 Fix: v5.26 firstMissAt fix was deployed on 2026-07-13 but tools pages still
 *     loaded monetag-manager.v525.js (no firstMissAt logic). BI 7d post-deploy showed
 *     0 freshReset events on tools (vs 5 on gz.com which uses /monetag-manager.js
 *     directly). Users never got the v5.26 fix → amplification stayed at 1.45x.
 *   - 🔧 Create monetag-manager.v526.js (copy of monetag-manager.js with VERSION
 *     bumped). shared/common.js now loads v526.js?v=v526backoffreset forcing browser
 *     fresh fetch. v525 file kept for forensic compare.
 *   - 📊 Expected impact (BI 7d post-deploy):
 *       - tools 11012002 zone_backoff_skip amplification: 1.45x → 0.5-1.0x
 *       - tools 11012002 fill rate: ~30% (current) → 40-55%
 *   - 🛡️ Safety: VERSION bump only, code identical to v5.26. Revert by setting
 *     common.js back to v525.js?v=v525adsenseslotfix.
 *   - Version bumped 5.26-tools-backoff-fresh-reset → 5.27-tools-v526-bump.
 *
...
;(function() {
  ...
  var CONFIG = {
    ...
    VERSION: '5.27-tools-v526-bump',  // 2026-07-14: v5.27 kanban t_40a17bfc — create monetag-manager.v526.js + bump common.js cache version. v5.26 firstMissAt logic was deployed 7-13 but tools pages still loaded v525.js → users never got the fix (tools freshReset=0/wk vs gz.com 5/wk). Code identical to v5.26, only VERSION + changelog bumped.
